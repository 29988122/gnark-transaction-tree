...
func main() {
	...
	// Step 12: Compile the circuit using Polyhedra's Go ExpanderCompiler.
	ccs, err := ecgo.Compile(ecc.BN254.ScalarField(), &circuitDef)
	if err != nil {
		log.Fatalf("Failed to compile circuit: %v", err)
	}
	c := ccs.GetLayeredCircuit()
	os.WriteFile("circuit.txt", c.Serialize(), 0o644)

	// Step 13: Create a witness.
	inputSolver := ccs.GetInputSolver()
	witness, _ := inputSolver.SolveInputAuto(&circuitDef)
	os.WriteFile("witness.txt", witness.Serialize(), 0o644)

	// Step 14: Verify the integrity of the generated witness and circuit.
